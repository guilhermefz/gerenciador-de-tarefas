...
Abra `server/src/utils/task.utils.ts` e `server/src/utils/auth.utils.ts`. São seis funções no total, e todas estão sendo chamadas em algum ponto da aplicação:
...
Crie o arquivo `server/src/tests/services/task.utils.test.ts`:
...
```bash
cd server
npx jest task.utils.test.ts
```

Saída esperada:

```
 PASS  src/tests/services/task.utils.test.ts
  formatarPrioridade
...
```
 FAIL  src/tests/services/task.utils.test.ts
  formatarPrioridade
    ✕ deve retornar "Alta" para prioridade "high"
...
Corrija o teste e introduza um defeito na função. Abra `task.utils.ts` e mude temporariamente:
...
Corrija o arquivo `task.utils.ts` antes de continuar.
...
> **Checkpoint:** Devem passar 9 testes (5 anteriores + 4 novos). Se algum falhou, leia a implementação de `estaAtrasada` em `task.utils.ts` e ajuste as expectativas.
...
Abra `server/src/routes/task.routes.ts`. As rotas disponíveis:
...
**Pontos de apoio:**
- Confirme as rotas em `server/src/routes/task.routes.ts`
- Status de sucesso para GET é `200` (ou `StatusCodes.OK`)
- Veja em `server/src/controllers/task.controller.ts` o que o *endpoint* retorna em caso de erro
...
No arquivo `task.utils.test.ts`, adicione `describe('validarEmail', () => {...})` com testes para:
